        ...
        for row_offset in range(-1, 2, 1):
            for col_offset in range(-1, 2, 1):
                if abs(row_offset) != abs(col_offset):
                    ...
                    if (0 <= r + row_offset < len(self.map)) and (
                        0 <= c + col_offset < len(self.map[0])
                    ):
                        # if the number is not a 9 and not already in the list, scan that cell
                        if (
                            self.map[r + row_offset][c + col_offset] != 9
                            and (r + row_offset, c + col_offset) not in current_basin
                        ):
                            current_basin.append((r + row_offset, c + col_offset))
                            self.check_basins(
                                (r + row_offset, c + col_offset), current_basin
                            )
